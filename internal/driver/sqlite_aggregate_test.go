...
func TestSQLiteAggregate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			defer db.Close()
			aggExecSetup(t, db, tt.setup)
			got, queryErr := aggCollectRows(t, db, tt.query)
			if tt.wantErr {
				if queryErr == nil {
			...
			aggCompareResults(t, got, tt.want)
		})
	}
}

func aggExecSetup(t *testing.T, db *sql.DB, stmts []string) {
	t.Helper()
...
func aggCollectRows(t *testing.T, db *sql.DB, query string) ([][]interface{}, error) {
	t.Helper()
...
func aggCompareResults(t *testing.T, got, want [][]interface{}) {
	t.Helper()
	if len(got) != len(want) {
	...
	for i, wantRow := range want {
		aggCompareRow(t, i, got[i], wantRow)
	}
}

func aggCompareRow(t *testing.T, i int, gotRow, wantRow []interface{}) {
	t.Helper()
	...
	for j, wantVal := range wantRow {
		if !compareAggregateValues(gotRow[j], wantVal) {
...
func sanitizeFilename(name string) string {
	result := strings.Map(sanitizeFilenameRune, name)
	if len(result) > 50 {
		result = result[:50]
	}
	return result
...
func sanitizeFilenameRune(c rune) rune {
