...
func TestSQLiteHaving(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			defer db.Close()
			havingExecSetup(t, db, tt.setup)
			got, queryErr := havingCollectRows(t, db, tt.query)
			if tt.wantErr {
				if queryErr == nil {
			...
			havingCompareResults(t, got, tt.want)
		})
	}
}

func havingExecSetup(t *testing.T, db *sql.DB, stmts []string) {
	t.Helper()
...
func havingCollectRows(t *testing.T, db *sql.DB, query string) ([][]interface{}, error) {
	t.Helper()
...
func havingCompareResults(t *testing.T, got, want [][]interface{}) {
	t.Helper()
	if len(got) != len(want) {
	...
	for i, wantRow := range want {
		havingCompareRow(t, i, got[i], wantRow)
	}
}

func havingCompareRow(t *testing.T, i int, gotRow, wantRow []interface{}) {
	t.Helper()
	...
	for j, wantVal := range wantRow {
		if !compareHavingValues(gotRow[j], wantVal) {
