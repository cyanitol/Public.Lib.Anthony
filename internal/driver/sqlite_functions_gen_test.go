...
// ---------- LAG / LEAD / FIRST_VALUE / LAST_VALUE window function tests ----------

// windowValueTestCase describes one LAG/LEAD/FIRST_VALUE/LAST_VALUE test.
type windowValueTestCase struct {
	name  string
	setup []string
	query string
	want  string
...
// generateLagTests builds LAG() test cases.
func generateLagTests() []windowValueTestCase {
	setup := []string{
		"CREATE TABLE wlag(id INTEGER, val TEXT)",
		"INSERT INTO wlag VALUES(1, 'a')",
		"INSERT INTO wlag VALUES(2, 'b')",
		"INSERT INTO wlag VALUES(3, 'c')",
		"INSERT INTO wlag VALUES(4, 'd')",
	}

	return []windowValueTestCase{
		{
			name:  "lag_default_offset",
			setup: setup,
			query: "SELECT COALESCE(LAG(val) OVER (ORDER BY id), 'NULL') FROM wlag",
			want:  "NULL a b c",
		},
		{
			name:  "lag_offset_1",
			setup: setup,
			query: "SELECT COALESCE(LAG(val, 1) OVER (ORDER BY id), 'NULL') FROM wlag",
			want:  "NULL a b c",
		},
		{
			name:  "lag_offset_2",
			setup: setup,
			query: "SELECT COALESCE(LAG(val, 2) OVER (ORDER BY id), 'NULL') FROM wlag",
			want:  "NULL NULL a b",
		},
	}
}

// generateLeadTests builds LEAD() test cases.
func generateLeadTests() []windowValueTestCase {
	setup := []string{
		"CREATE TABLE wlead(id INTEGER, val TEXT)",
		"INSERT INTO wlead VALUES(1, 'a')",
		"INSERT INTO wlead VALUES(2, 'b')",
		"INSERT INTO wlead VALUES(3, 'c')",
		"INSERT INTO wlead VALUES(4, 'd')",
	}
	...
	return []windowValueTestCase{
		{
			...
			setup: setup,
			query: "SELECT COALESCE(LEAD(val) OVER (ORDER BY id), 'NULL') FROM wlead",
			want:  "b c d NULL",
		},
		{
			name:  "lead_offset_1",
			setup: setup,
			query: "SELECT COALESCE(LEAD(val, 1) OVER (ORDER BY id), 'NULL') FROM wlead",
			want:  "b c d NULL",
		},
		{
			name:  "lead_offset_2",
			setup: setup,
			query: "SELECT COALESCE(LEAD(val, 2) OVER (ORDER BY id), 'NULL') FROM wlead",
			want:  "c d NULL NULL",
		},
	}
}

// generateFirstValueTests builds FIRST_VALUE() test cases.
func generateFirstValueTests() []windowValueTestCase {
	setup := []string{
		"CREATE TABLE wfv(id INTEGER, val TEXT)",
		"INSERT INTO wfv VALUES(1, 'a')",
		"INSERT INTO wfv VALUES(2, 'b')",
		"INSERT INTO wfv VALUES(3, 'c')",
	}

	return []windowValueTestCase{
		{
			name:  "first_value_unbounded",
			setup: setup,
			query: "SELECT FIRST_VALUE(val) OVER (ORDER BY id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) FROM wfv",
			want:  "a a a",
		},
	}
}

// generateLastValueTests builds LAST_VALUE() test cases.
func generateLastValueTests() []windowValueTestCase {
	setup := []string{
		"CREATE TABLE wlv(id INTEGER, val TEXT)",
		"INSERT INTO wlv VALUES(1, 'a')",
		"INSERT INTO wlv VALUES(2, 'b')",
		"INSERT INTO wlv VALUES(3, 'c')",
	}
	...
	return []windowValueTestCase{
		{
			...
			setup: setup,
			query: "SELECT LAST_VALUE(val) OVER (ORDER BY id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) FROM wlv",
			want:  "c c c",
		},
	}
}

// TestWindowFunctionsLag tests LAG() window function.
func TestWindowFunctionsLag(t *testing.T) {
	for _, tc := range generateLagTests() {
		t.Run(tc.name, func(t *testing.T) {
			runWindowValueCase(t, tc)
		})
	}
}

// TestWindowFunctionsLead tests LEAD() window function.
func TestWindowFunctionsLead(t *testing.T) {
	for _, tc := range generateLeadTests() {
		t.Run(tc.name, func(t *testing.T) {
			runWindowValueCase(t, tc)
		})
	}
...
// TestWindowFunctionsFirstValue tests FIRST_VALUE() window function.
func TestWindowFunctionsFirstValue(t *testing.T) {
	for _, tc := range generateFirstValueTests() {
		t.Run(tc.name, func(t *testing.T) {
			runWindowValueCase(t, tc)
		})
	}
...
// TestWindowFunctionsLastValue tests LAST_VALUE() window function.
func TestWindowFunctionsLastValue(t *testing.T) {
	for _, tc := range generateLastValueTests() {
		t.Run(tc.name, func(t *testing.T) {
			runWindowValueCase(t, tc)
		})
	}
