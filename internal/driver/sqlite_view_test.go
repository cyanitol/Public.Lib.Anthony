...
func TestSQLiteViewErrors(t *testing.T) {
	tests := []struct {
		...
		{
			name: "view-2.2 cannot insert into view",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
				"CREATE VIEW v2 AS SELECT * FROM t1 WHERE a>5",
			},
			query:  "INSERT INTO v2 VALUES(1,2,3,4)",
			errMsg: "cannot modify v2 because it is a view",
		},
		{
			name: "view-2.3 cannot update view",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
				"CREATE VIEW v2 AS SELECT * FROM t1 WHERE a>5",
			},
			query:  "UPDATE v2 SET a=10 WHERE a=5",
			errMsg: "cannot modify v2 because it is a view",
		},
		{
			name: "view-2.4 cannot delete from view",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
				"CREATE VIEW v2 AS SELECT * FROM t1 WHERE a>5",
			},
			query:  "DELETE FROM v2",
			errMsg: "cannot modify v2 because it is a view",
		},
		{
			name: "view-3.3.5 column count mismatch - too few columns",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
				"CREATE VIEW v1err(x,y) AS SELECT a, b+c, c-b FROM t1",
			},
			query:  "SELECT * FROM v1err",
			errMsg: "expected 2 columns",
		},
		{
			name: "view-3.3.6 column count mismatch - too many columns",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
			...
			query:  "SELECT * FROM v1err",
			errMsg: "expected 4 columns",
		},
		{
			name: "view-4.1 cannot drop table with drop view",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "DROP VIEW t1",
		...
		{
			name: "view-4.3 cannot drop view with drop table",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
		...
		{
			name: "view-4.5 cannot create index on view",
			setup: []string{
				"CREATE TABLE t1(x,a,b,c)",
				"INSERT INTO t1 VALUES(1,2,3,4)",
				"CREATE VIEW v1 AS SELECT a AS 'xyz', b+c AS 'pqr', c-b FROM t1",
			},
			query:  "CREATE INDEX i1v1 ON v1(xyz)",
			errMsg: "views may not be indexed",
		},
		{
			name: "view-12.1 parameters not allowed in views",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "CREATE VIEW v12 AS SELECT a FROM t1 WHERE b=?",
			errMsg: "parameters are not allowed in views",
		},
		{
			name: "view-12.2 parameters not allowed in views with column list",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "CREATE VIEW v12(x) AS SELECT a FROM t1 WHERE b=?",
			errMsg: "parameters are not allowed in views",
		},
		{
			name: "view-14.1 circular view definition",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "CREATE TEMP VIEW t1 AS SELECT a,b FROM t1",
			errMsg: "view t1 is circularly defined",
		},
		{
			name: "view-14.2 circular view definition with column list",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "CREATE TEMP VIEW t1(a,b) AS SELECT a,b FROM t1",
			errMsg: "view t1 is circularly defined",
		},
		{
			name: "view-17.1 drop non-existent view",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "DROP VIEW nosuchview",
			errMsg: "no such view: nosuchview",
		},
		{
			name: "view-17.2 drop non-existent view with schema",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "DROP VIEW main.nosuchview",
			errMsg: "no such view: main.nosuchview",
		},
		{
			name: "view-29.0 syntax error in view name",
			setup: []string{
				"CREATE TABLE t1(a,b,c)",
			},
			query:  "CREATE VIEW IF NOT EXISTS IF AS SELECT null",
			errMsg: "syntax error",
		},
	}
...
func TestSQLiteViewInsteadOfTriggers(t *testing.T) {
	...
	_, err := db.Exec(`
		...
		CREATE TABLE addresses (id INTEGER PRIMARY KEY, user_id INTEGER, address TEXT);
	`)
	if err != nil {
	...
	_, err = db.Exec(`
		...
		BEGIN
			...
			INSERT INTO addresses (user_id, address) VALUES (last_insert_rowid(), NEW.address);
		END
	`)
	if err != nil {
	...
	// Note: Trigger execution is not yet fully implemented in this driver
	// This test validates that the trigger can be created successfully
	// Once trigger execution is implemented, we would test:
	// INSERT INTO user_addresses (name, address) VALUES ('Alice', '123 Main St')
	// and verify both tables are updated correctly
}
