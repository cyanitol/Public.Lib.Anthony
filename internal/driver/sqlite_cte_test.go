...
func buildCTETests() []cteTestCase {
	return []cteTestCase{
		...
		// Recursive ancestors
		{
			...
			query: "WITH RECURSIVE ancestors(id, parent_id, name) AS (SELECT id, parent_id, name FROM person WHERE id = 3 UNION ALL SELECT p.id, p.parent_id, p.name FROM person p JOIN ancestors a ON p.id = a.parent_id) SELECT name FROM ancestors ORDER BY id",
			wantRows: [][]interface{}{
				{"Grandparent"},
				{"Parent"},
				{"Child"},
		...
		// Recursive binary tree traversal
		{
			...
			query:   "WITH RECURSIVE traverse(id, value) AS (SELECT id, value FROM btree WHERE id = 1 UNION ALL SELECT b.id, b.value FROM btree b JOIN traverse t ON b.id = t.left_id OR b.id = t.right_id) SELECT value FROM traverse",
			wantErr: true,
		},
		// CTE with IN clause
		{
		...
		// CTE with MIN/MAX aggregates
		{
			...
			setup: []string{
				"CREATE TABLE val_data(category TEXT, amount REAL)",
				"INSERT INTO val_data VALUES('A', 10.0)",
				"INSERT INTO val_data VALUES('A', 20.0)",
				"INSERT INTO val_data VALUES('B', 15.0)",
				"INSERT INTO val_data VALUES('B', 25.0)",
			},
			query: "WITH ranges AS (SELECT category, MIN(amount) AS min_val, MAX(amount) AS max_val FROM val_data GROUP BY category) SELECT category, min_val, max_val FROM ranges ORDER BY category",
			wantRows: [][]interface{}{
				{"A", int64(10), int64(20)},
				{"B", int64(15), int64(25)},
			},
