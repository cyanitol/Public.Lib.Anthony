...
func TestAttachGenCreateTableInAttached(t *testing.T) {
	...
	if err != nil {
		t.Skipf("CREATE TABLE in attached db not yet wired: %v", err)
	}
}
...
func TestAttachGenCrossDBSelect(t *testing.T) {
	...
	if err != nil {
		t.Skipf("INSERT into attached db not yet wired: %v", err)
	}

	var val int
	err = db.QueryRow("SELECT a FROM aux.t1").Scan(&val)
	if err != nil {
		t.Skipf("SELECT from attached db not yet wired: %v", err)
	}
	if val != 42 {
...
func TestAttachGenPragmaDatabaseList(t *testing.T) {
	...
	if err != nil {
		t.Skipf("PRAGMA database_list not yet wired: %v", err)
	}
	defer rows.Close()
	...
	if len(names) == 0 {
		t.Skip("PRAGMA database_list returns no rows - not yet implemented")
	}
