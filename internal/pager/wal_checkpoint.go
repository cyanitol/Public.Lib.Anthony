...
func (w *WAL) checkpointRestart() (int, int, error) {
	...
	if err := w.file.Truncate(0); err != nil {
		return len(pageFrames), 0, fmt.Errorf("failed to truncate WAL: %w", err)
	}

	// Seek to beginning and write fresh header
	if _, err := w.file.Seek(0, 0); err != nil {
		return len(pageFrames), 0, fmt.Errorf("failed to seek WAL: %w", err)
	}
	...
	if err := w.writeHeader(); err != nil {
		return len(pageFrames), 0, fmt.Errorf("failed to write new WAL header: %w", err)
	}

	return len(pageFrames), 0, nil
}
...
func (w *WAL) checkpointTruncate() (int, int, error) {
	...
	// Open database file if not already open (even if no frames)
	if w.dbFile == nil && initialFrameCount > 0 {
		var err error
		w.dbFile, err = os.OpenFile(w.dbFilename, os.O_RDWR, 0600)
		if err != nil {
			return 0, initialFrameCount, fmt.Errorf("failed to open database file: %w", err)
	...
	if err := os.Truncate(w.filename, 0); err != nil {
		return framesCheckpointed, 0, fmt.Errorf("failed to truncate WAL file: %w", err)
	}
