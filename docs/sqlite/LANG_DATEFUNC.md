...
ISO-8601
A text string that is one of the ISO 8601 date/time values
shown in items 1 through 10 below.
Example: '2025-05-29 14:16:00'
Julian day number
...
Example: 1748528160
These three formats are collectively known as time-values.
...
adding optional modifier arguments 'auto' or
'unixepoch'.
...
the subsec modifier is used.
The datetime() function returns the date and time formatted as YYYY-MM-DD HH:MM:SS or
as YYYY-MM-DD HH:MM:SS.SSS if the subsec modifier is used.
...
an integer number of seconds, but with the optional subsec modifier it
...
ISO-8601 supports a wide variety of
...
The available modifiers are as follows.
 ...
 &plusmn;HH:MM:SS.SSS
 &plusmn;YYYY-MM-DD
 &plusmn;YYYY-MM-DD HH:MM
 &plusmn;YYYY-MM-DD HH:MM:SS
 &plusmn;YYYY-MM-DD HH:MM:SS.SSS
 ceiling
 floor
 start of month
 start of year
...
for formats 7, 8, and 9.  The changes are applied from left to right.
...
datetime(), time(), and unixepoch(), and for the "%s"
format string in strftime().  The "subsecond"
...
datetime() or time(), the seconds field at the end is
followed by a decimal point and one or more digits to show
fractional seconds.  When "subsec" is used with unixepoch(),
