...
Note: The original SQLite documentation includes detailed syntax diagrams (railroad diagrams)
for these constructs. Please refer to the online HTML documentation for the visual syntax diagrams.

Syntax constructs defined:
- expr
- filter-clause
- function-arguments
- ordering-term
...
- signed-number
## 2. Operators, and Parse-Affecting Attributes
SQLite understands these operators, listed in precedence order
...
| `[expr] BETWEEN [expr] AND [expr]` `IN` `MATCH` `LIKE` `REGEXP` `GLOB` |
...
- `[expr]` denotes operand locations for non-binary operators. Operators with no `[expr]` adjunct are binary and left-associative.
- The COLLATE clause (with its collation-name) acts as a single postfix operator.
- The ESCAPE clause (with its escape character) acts as a single postfix operator. It can only bind to a preceding `[expr] LIKE [expr]` expression.
- Each keyword in (BETWEEN IN GLOB LIKE MATCH REGEXP) may be prefixed by NOT, retaining the bare operator's precedence and associativity.
...
notation. Those compact forms are an SQLite extension. You must use
the less readable IS NOT DISTINCT FROM and
IS DISTINCT FROM operators in most other SQL database engines.
## 3. Literal Values (Constants)
...
is TRUE but **'ae' LIKE 'AE'** is FALSE.
...
case the right-hand side is understood to be a subquery of
...
| Left operand is NULL | Right operand contains NULL | Right operand is empty set | Left operand found within right operand | Result of IN operator | Result of NOT IN operator |
| --- | --- | --- | --- | --- | --- |
...
the CAST operator the conversion always takes place even if the conversion is
