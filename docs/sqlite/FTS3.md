...
table.
### Differences between FTS3 and FTS4
...
[matchinfo()](fts3.html#matchinfo) function.
-
...
### Creating and Destroying FTS Tables
...
body TEXT CHECK(length(body) " in place of a column
name, where  is the name of the tokenizer to use and
is an optional list of whitespace separated qualifiers
...
form "INSERT INTO () VALUES('optimize')",
...
a sub-clause of the form " MATCH ?", FTS is able to use
...
-
The [FTS MATCH](fts3.html#full_text_index_queries) operator is supported for queries based on the built-in
full-text index.
-
The [FTS auxiliary functions](fts3.html#snippet), [snippet()](fts3.html#snippet), [offsets()](fts3.html#offsets), and [matchinfo()](fts3.html#matchinfo) are
available to support full-text queries.
-
Every FTS table has a [hidden column](vtab.html#hiddencol) with the
...
the preprocessor macro [SQLITE_ENABLE_FTS3](compile.html#enable_fts3) when compiling. New applications
should also define the [SQLITE_ENABLE_FTS3_PARENTHESIS](compile.html#enable_fts3_parenthesis) macro to enable the
...
```sql
CPPFLAGS="-DSQLITE_ENABLE_FTS3_PARENTHESIS" ./configure --enable-fts3
```
where ** are those options normally passed to
the configure script, if any.
Because FTS3 and FTS4 are virtual tables, The [SQLITE_ENABLE_FTS3](compile.html#enable_fts3) compile-time option
is incompatible with the [SQLITE_OMIT_VIRTUALTABLE](compile.html#omit_virtualtable) option.
...
an operator of the form "NEAR/**" may be used, where
** is the maximum number of intervening terms allowed.
For example:
```sql
*-- Virtual table declaration.*
...
```
### Set Operations Using The Standard Query Syntax
...
```sql
SELECT offsets(mail) FROM mail WHERE mail MATCH ;
SELECT snippet(mail) FROM mail WHERE mail MATCH ;
SELECT matchinfo(mail) FROM mail WHERE mail MATCH ;
```
...
| 0  | N/A
|  The first argument to the snippet function must always be the [FTS hidden column](fts3.html#hiddencol)
of the FTS table being queried and from which the snippet is to be taken.  The
[FTS hidden column](fts3.html#hiddencol) is an automatically generated column with the same name as the
FTS table itself.
| 1  | ""
...
*--*
*--   "...cool elsewhere, minimum temperature 17-20oC. Cold to very *
*--    cold on mountaintops, minimum temperature 6...".*
*--*
...
table (i.e. not including the docid or the [FTS hidden column](fts3.html#hiddencol)).
| x  | 3 * *cols* * *phrases*
|
...
[matchinfo 'x' flag](fts3.html#matchinfo-x). However, the number of hits reported by the
...
"a c d"
```
The [matchinfo 'x' flag](fts3.html#matchinfo-x) would report a single hit for the phrases "a" and "c".
...
[matchinfo 'y' flag](fts3.html#matchinfo-y), but in a more
...
```
p_is_in_c = array&#91;p * ((nCol+31)/32)&#93; & (1 ;
SELECT docid, offsets(tbl) FROM tbl WHERE tbl MATCH ;
```
...
value.
| languageid *(hidden)* |
This column determines which [languageid](fts3.html#*fts4languageid) is used to
...
[matchinfo()](fts3.html#matchinfo) is no longer available.
| notindexed |
...
in the database file to optimize [prefix queries](fts3.html#termprefix) where
the query term is N bytes in length, not including the '*' character,
when encoded using UTF-8. [See below](fts3.html#the_prefix_option) for details.
| uncompress |
...
of an SQL scalar function registered using [sqlite3_create_function()](c3ref/create_function.html)
...
that use other operators (e.g.  lid!=5, or lid
### The matchinfo= option
...
```
## Special Commands For FTS3 and FTS4
...
[PRAGMA integrity_check](pragma.html#pragma_integrity_check).  In a working system, the "integrity-command"
...
command was originally conceived as part of the test suite
for FTS3/4.)
-  Corruption to the underlying SQLite database file.  (See
documentation on [how to corrupt](howtocorrupt.html) an SQLite database for
additional information.)
### The "merge=X,Y" command
...
in [sqlite3_total_changes()](c3ref/total_changes.html) before and after each "merge=X,Y"
command and stopping the loop when the difference drops below two.
### The "automerge=N" command
...
### Custom (Application Defined) Tokenizers
...
SELECT fts3_tokenizer(, );
```
Where  is a [parameter](lang_expr.html#varparam) to which a string is bound
using [sqlite3_bind_text()](c3ref/bind_blob.html) where the string identifies the tokenizer and
is a [parameter](lang_expr.html#varparam) to which a BLOB is
bound using [sqlite3_bind_blob()](c3ref/bind_blob.html) where the value of the BLOB is a
...
that tokenizer.
## Data Structures
This overview describes how the FTS module stores its
...
## Limitations
###  UTF-16 byte-order-mark problem
...
-  All text strings are inserted using one of the
[sqlite3_bind_text()](c3ref/bind_blob.html) family of functions.
...
to occur.  And even if all of the conditions above are false,
most things will still operate correctly.  Only the
[integrity-check](#integcheck) command and the
[auxiliary functions](#snippet) might give
unexpected results.
...
calculated and stored in a temporary table before the main query is executed.
...
-
The [snippet](fts3.html#snippet) function cannot be used with the above query. Because
...
[sqlite.org documentation search](https://sqlite.org/search?q=fts3)
application.
```sql
...
with SQLite like any other user function using [sqlite3_create_function](c3ref/create_function.html).
