...
[PRAGMA legacy_alter_table=ON](pragma.html#pragma_legacy_alter_table) statement or the
[SQLITE_DBCONFIG_LEGACY_ALTER_TABLE](c3ref/c_dbconfig_defensive.html#sqlitedbconfiglegacyaltertable) configuration parameter
on [sqlite3_db_config()](c3ref/db_config.html) interface to make ALTER TABLE RENAME
...
[PRAGMA foreign_keys=ON](pragma.html#pragma_foreign_keys), or in other words if
[foreign key constraints](foreignkeys.html) were being enforced.  With
[PRAGMA foreign_keys=OFF](pragma.html#pragma_foreign_keys), FOREIGN KEY constraints would not be changed
...
[PRAGMA legacy_alter_table=ON](pragma.html#pragma_legacy_alter_table) setting is engaged.  The following
...
stored in the [sqlite_schema table](schematab.html).
...
SQLite stores the schema as plain text in the [sqlite_schema table](schematab.html).
...
any entries in the [sqlite_schema table](schematab.html) that do not parse.  For
...
can be disabled by setting "[PRAGMA writable_schema=ON;](pragma.html#pragma_writable_schema)".  When
...
If foreign key constraints are enabled, disable them using [PRAGMA foreign_keys=OFF](pragma.html#pragma_foreign_keys).
...
Use [CREATE TABLE](lang_createtable.html) to construct a new table "new_X" that is in the desired
...
Drop the old table X:  [DROP TABLE X](lang_droptable.html).
...
Use [CREATE INDEX](lang_createindex.html), [CREATE TRIGGER](lang_createtrigger.html), and [CREATE VIEW](lang_createview.html)
...
then run [PRAGMA foreign_key_check](pragma.html#pragma_foreign_key_check) to verify that the schema
...
- 
 Run [PRAGMA schema_version](pragma.html#pragma_schema_version) to determine the current schema
version number.  This number will be needed for step 6 below.

- 
 Activate schema editing using 
[PRAGMA writable_schema=ON](pragma.html#pragma_writable_schema).

- 
 Run an [UPDATE](lang_update.html) statement to change the definition of table X
in the [sqlite_schema table](schematab.html): 
...
[PRAGMA schema_version=X](pragma.html#pragma_schema_version) where X is one
more than the old schema version number found in step 2 above.

- 
 Disable schema editing using 
[PRAGMA writable_schema=OFF](pragma.html#pragma_writable_schema).

- 
 (Optional) Run [PRAGMA integrity_check](pragma.html#pragma_integrity_check) to verify that the
...
in the [sqlite_schema](schematab.html) table as the original text of the CREATE
