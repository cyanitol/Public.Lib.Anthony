...
are determined by counting "groups" relative to the current group.
A "group" is a set of rows that all have equivalent values for all
terms of the window ORDER BY clause.  ("Equivalent" means that
...
window has exactly one term.  Call that term "X".  With the
...
-  **xFinal()** - invoked to reclaim any allocated resources and to
                         obtain the value for row (x='e'). 9.     .
If the user were to abandon query execution by calling sqlite3_reset() or
