...
The sqlite3_step() routine returns SQLITE_ROW if it is returning a single
...
It is not necessary to retrieve data in the format specified by
...
The database engine chooses the function that minimizes the number
