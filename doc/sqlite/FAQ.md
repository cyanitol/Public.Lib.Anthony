...
- [How do I create an AUTOINCREMENT field?](#q1)
- [What datatypes does SQLite support?](#q2)
- [SQLite lets me insert a string into a database column of type integer!](#q3)
...
- [Is SQLite threadsafe?](#q6)
- [How do I list all tables/indices contained in an SQLite database](#q7)
- [Are there any known size limits to SQLite databases?](#q8)
- [What is the maximum size of a VARCHAR in SQLite?](#q9)
- [Does SQLite support a BLOB type?](#q10)
- [How do I add, delete or rename columns from an existing table in SQLite?](#q11)
- [I deleted a lot of data but the database file did not get any
  smaller.  Is this a bug?](#q12)
- [Can I use SQLite in my commercial product without paying royalties?](#q13)
- [How do I use a string literal that contains an embedded single-quote (')
  character?](#q14)
- [What is an SQLITE_SCHEMA error, and why am I getting one?](#q15)
- [I get some compiler warnings when I compile SQLite.
  Isn't this a problem?  Doesn't it indicate poor code quality?](#q17)
- [Case-insensitive matching of Unicode characters does not work.](#q18)
- [INSERT is really slow - I can only do few dozen INSERTs per second](#q19)
...
- [Does SQLite support foreign keys?](#q22)
...
- [What is the Export Control Classification Number (ECCN) for SQLite?](#q27)
...
There is a function named
  [sqlite3_last_insert_rowid()](c3ref/last_insert_rowid.html) which will return the integer key
...
An [SQLITE_SCHEMA](rescode.html#schema) error is returned when a 
  ...
  the [sqlite3_prepare()](c3ref/prepare.html) API.
  An SQLITE_SCHEMA error can only occur when using the [sqlite3_prepare()](c3ref/prepare.html),
  and [sqlite3_step()](c3ref/step.html) interfaces to run SQL.
  You will never receive an [SQLITE_SCHEMA](rescode.html#schema) error from
  [sqlite3_exec()](c3ref/exec.html).  Nor will you receive an error if you
  prepare statements using [sqlite3_prepare_v2()](c3ref/prepare.html) instead of
  [sqlite3_prepare()](c3ref/prepare.html).

  
The [sqlite3_prepare_v2()](c3ref/prepare.html) interface creates a
...
Instead of providing full Unicode case support by default, 
  ...
  sequence (using [sqlite3_create_collation()](c3ref/create_collation.html)) and the built-in
  [like()](lang_corefunc.html#like), [upper()](lang_corefunc.html#upper), and [lower()](lang_corefunc.html#lower) functions
  (using [sqlite3_create_function()](c3ref/create_function.html)).  
...
If you do not have a backup, recovery is very difficult.  You might
  ...
  with the [SQLITE_SECURE_DELETE](compile.html#secure_delete) option which overwrites all deleted content
...
An [SQLITE_CORRUPT](rescode.html#corrupt) error is returned when SQLite detects an error
...
You can use [PRAGMA integrity_check](pragma.html#pragma_integrity_check) 
  to do a thorough but time intensive test of the database integrity.

  
You can use [PRAGMA quick_check](pragma.html#pragma_quick_check) to do a faster 
...
The [SQLITE_OMIT_...](compile.html#omitfeatures) compile-time options only work
