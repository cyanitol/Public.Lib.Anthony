...
[1.1. Hot Journals](#11-hot-journals)
[1.2. Pages](#12-pages)
[1.3. The Database Header](#13-the-database-header)
[1.3.1. Magic Header String](#131-magic-header-string)
[1.3.2. Page Size](#132-page-size)
[1.3.3. File format version numbers](#133-file-format-version-numbers)
[1.3.4. Reserved bytes per page](#134-reserved-bytes-per-page)
[1.3.5. Payload fractions](#135-payload-fractions)
[1.3.6. File change counter](#136-file-change-counter)
[1.3.7. In-header database size](#137-in-header-database-size)
[1.3.8. Free page list](#138-free-page-list)
[1.3.9. Schema cookie](#139-schema-cookie)
[1.3.10. Schema format number](#1310-schema-format-number)
[1.3.11. Suggested cache size](#1311-suggested-cache-size)
[1.3.12. Incremental vacuum settings](#1312-incremental-vacuum-settings)
[1.3.13. Text encoding](#1313-text-encoding)
[1.3.14. User version number](#1314-user-version-number)
[1.3.15. Application ID](#1315-application-id)
[1.3.16. Write library version number and version-valid-for number](#1316-write-library-version-number-and-version-valid-for-number)
[1.3.17. Header space reserved for expansion](#1317-header-space-reserved-for-expansion)
[1.4. The Lock-Byte Page](#14-the-lock-byte-page)
[1.5. The Freelist](#15-the-freelist)
[1.6. B-tree Pages](#16-b-tree-pages)
[1.7. Cell Payload Overflow Pages](#17-cell-payload-overflow-pages)
[1.8. Pointer Map or Ptrmap Pages](#18-pointer-map-or-ptrmap-pages)
[2. Schema Layer](#2-schema-layer)
[2.1. Record Format](#21-record-format)
[2.2. Record Sort Order](#22-record-sort-order)
[2.3. Representation Of SQL Tables](#23-representation-of-sql-tables)
[2.4. Representation of WITHOUT ROWID Tables](#24-representation-of-without-rowid-tables)
...
[2.5. Representation Of SQL Indices](#25-representation-of-sql-indices)
[2.5.1. Suppression of redundant columns in WITHOUT ROWID secondary indexes
](#suppression_of_redundant_columns_in_without_rowid_secondary_indexes_)
[2.6. Storage Of The SQL Database Schema](#26-storage-of-the-sql-database-schema)
[2.6.1. Alternative Names For The Schema Table](#261-alternative-names-for-the-schema-table)
[2.6.2. Internal Schema Objects](#262-internal-schema-objects)
[2.6.3. The sqlite_sequence table](#263-the-sqlite_sequence-table)
[2.6.4. The sqlite_stat1 table](#264-the-sqlite_stat1-table)
[2.6.5. The sqlite_stat2 table](#265-the-sqlite_stat2-table)
[2.6.6. The sqlite_stat3 table](#266-the-sqlite_stat3-table)
[2.6.7. The sqlite_stat4 table](#267-the-sqlite_stat4-table)
[3. The Rollback Journal](#3-the-rollback-journal)
[4. The Write-Ahead Log](#4-the-write-ahead-log)
[4.1. WAL File Format](#41-wal-file-format)
[4.2. Checksum Algorithm](#42-checksum-algorithm)
[4.3. Checkpoint Algorithm](#43-checkpoint-algorithm)
[4.4. WAL Reset](#44-wal-reset)
[4.5. Reader Algorithm](#45-reader-algorithm)
[4.6. WAL-Index Format](#46-wal-index-format)
...
| 60 | 4 | 
The "user version" as read and set by the [user_version pragma](pragma.html#pragma_user_version).
 
| 64 | 4 | 
True (non-zero) for incremental-vacuum mode.  False (zero) otherwise.
 
| 68 | 4 | 
The "Application ID" set by [PRAGMA application_id](pragma.html#pragma_application_id).
...
| 96 | 4 | 
[SQLITE_VERSION_NUMBER](c3ref/c_scm_branch.html)
...
automatically reprepares and reruns or it aborts with an [SQLITE_SCHEMA](rescode.html#schema) 
error.
...
The [SQLITE_DBCONFIG_LEGACY_FILE_FORMAT](c3ref/c_dbconfig_defensive.html#sqlitedbconfiglegacyfileformat) option for the
[sqlite3_db_config()](c3ref/db_config.html) C-language interface can be used to cause SQLite
to create new database files using format 1. The format version number
can be made to default to 1 instead of 4 by setting
[SQLITE_DEFAULT_FILE_FORMAT](compile.html#default_file_format)=1 at compile-time.
...
can be set using the [default_cache_size pragma](pragma.html#pragma_default_cache_size).

### 1.3.12. Incremental vacuum settings

The two 4-byte big-endian integers at offsets 52 and 64 are used
to manage the [auto_vacuum](pragma.html#pragma_auto_vacuum) and [incremental_vacuum](pragma.html#pragma_incremental_vacuum) modes.  If
...
is set and queried by the [user_version pragma](pragma.html#pragma_user_version).  The user version is
not used by SQLite.

### 1.3.15. Application ID

The 4-byte big-endian integer at offset 68 is an "Application ID" that
can be set by the [PRAGMA application_id](pragma.html#pragma_application_id) command in order to identify the
...
[SQLITE_VERSION_NUMBER](c3ref/c_scm_branch.html) value for the SQLite library that most
...
such as [sqlite_schema](schematab.html).  There is one index b-tree
...
to make the operation of [auto_vacuum](pragma.html#pragma_auto_vacuum) and [incremental_vacuum](pragma.html#pragma_incremental_vacuum) modes
...
| N&#x2265;13 and odd
     | (N-13)/2 | 
Value is a string in the [text encoding](fileformat2.html#enc) and (N-13)/2 bytes in length.
...
SQLite using the [sqlite3_create_collation()](c3ref/create_collation.html) interface.
...
holds a special table named "[sqlite_schema](schematab.html)".  This b-tree is known
...
[internal indexes](fileformat2.html#intschema) with names of the form "sqlite_autoindex_TABLE_N"
...
In a [WITHOUT ROWID](withoutrowid.html) table, there is no sqlite_schema entry for the
...
The sqlite_schema.sql is NULL for the [internal indexes](fileformat2.html#intschema) that are
...
[AUTOINCREMENT](autoinc.html).  The sqlite_sequence table is created automatically
...
with an [SQLITE_FULL](rescode.html#full) error.
...
The sqlite_stat1 is an internal table created by the [ANALYZE](lang_analyze.html) command
...
with [SQLITE_ENABLE_STAT3](compile.html#enable_stat3) or [SQLITE_ENABLE_STAT4](compile.html#enable_stat4)
...
If the [SQLITE_ENABLE_STAT4](compile.html#enable_stat4) compile-time option is used and the
...
The [ANALYZE](lang_analyze.html) command will typically generate sqlite_stat3 tables
...
with [SQLITE_ENABLE_STAT4](compile.html#enable_stat4) and if the SQLite version number is
...
| idx: 
    ...
    an sqlite_stat4 entry for a [WITHOUT ROWID](withoutrowid.html) table, the
    name of the table itself.

 
| sample: 
    | The sqlite_stat4.sample column holds a BLOB
    in the [record format](fileformat2.html#record_format) that encodes the indexed columns followed by
...
The [ANALYZE](lang_analyze.html) command will typically generate sqlite_stat4 tables
that contain between 10 and 40 samples that are distributed across
the key space and with large nEq values.
...
TRUNCATE, and PERSIST settings, respectively, of the [journal_mode pragma](pragma.html#pragma_journal_mode).
...
[journal_mode=WAL](pragma.html#pragma_journal_mode) 
