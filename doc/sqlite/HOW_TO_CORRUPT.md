...
Table Of Contents
[1.  File overwrite by a rogue thread or process](#1-file-overwrite-by-a-rogue-thread-or-process)
[1.1.  Continuing to use a file descriptor after it has been closed](#11-continuing-to-use-a-file-descriptor-after-it-has-been-closed)
[1.2.  Backup or restore while a transaction is active](#12-backup-or-restore-while-a-transaction-is-active)
[1.3.  Deleting a hot journal](#13-deleting-a-hot-journal)
[1.4.  Mispairing database files and hot journals ](#14-mispairing-database-files-and-hot-journals)
[2.  File locking problems](#2-file-locking-problems)
[2.1.  Filesystems with broken or missing lock implementations](#21-filesystems-with-broken-or-missing-lock-implementations)
[2.2.  Posix advisory locks canceled by a separate thread doing close()](#22-posix-advisory-locks-canceled-by-a-separate-thread-doing-close)
[2.3. Multiple copies of SQLite linked into the same application](#23-multiple-copies-of-sqlite-linked-into-the-same-application)
[2.4.  Two processes using different locking protocols](#24-two-processes-using-different-locking-protocols)
[2.5.  Unlinking or renaming a database file while in use](#25-unlinking-or-renaming-a-database-file-while-in-use)
[2.6.  Multiple links to the same file](#26-multiple-links-to-the-same-file)
[2.7.  Carrying an open database connection across a fork() ](#27-carrying-an-open-database-connection-across-a-fork)
[3.  Failure to sync](#3-failure-to-sync)
[3.1.  Disk drives that do not honor sync requests](#31-disk-drives-that-do-not-honor-sync-requests)
[3.2.  Disabling sync using PRAGMAs](#32-disabling-sync-using-pragmas)
[4.  Disk Drive and Flash Memory Failures](#4-disk-drive-and-flash-memory-failures)
[4.1.  Non-powersafe flash memory controllers](#41-non-powersafe-flash-memory-controllers)
[4.2.  Fake capacity USB sticks](#42-fake-capacity-usb-sticks)
[5.  Memory corruption](#5-memory-corruption)
[6.  Other operating system problems](#6-other-operating-system-problems)
[6.1.  Linux Threads](#61-linux-threads)
[6.2.  Failures of mmap() on QNX](#62-failures-of-mmap-on-qnx)
[6.3.  Filesystem Corruption](#63-filesystem-corruption)
[7. SQLite Configuration Errors](#7-sqlite-configuration-errors)
[8.  Bugs in SQLite](#8-bugs-in-sqlite)
[8.1.  False corruption reports due to database shrinkage](#81-false-corruption-reports-due-to-database-shrinkage)
[8.2.  Corruption following switches between rollback and WAL modes](#82-corruption-following-switches-between-rollback-and-wal-modes)
[8.3.  I/O error while obtaining a lock leads to corruption](#83-io-error-while-obtaining-a-lock-leads-to-corruption)
[8.4.  Database pages leak from the free page list](#84-database-pages-leak-from-the-free-page-list)
[8.5.  Corruption following alternating writes from 3.6 and 3.7](#85-corruption-following-alternating-writes-from-36-and-37)
[8.6.  Race condition in recovery on Windows systems](#86-race-condition-in-recovery-on-windows-systems)
[8.7.  Boundary value error in the secondary journals used by nested transactions](#87-boundary-value-error-in-the-secondary-journals-used-by-nested-transactions)
...
[sqlite3_open_v2()](c3ref/open.html) so that the file descriptor used for the
...
for database files. 
(See [SQLITE_MINIMUM_FILE_DESCRIPTOR](compile.html#minimum_file_descriptor) for additional information.)
...
- 
      ...
      The [sqlite3_rsync](rsync.html) utility program (available beginning with
      SQLite 3.47.0 (2024-10-21) and later) will make a copy
      of a live SQLite over SSH using a bandwidth-efficient protocol.

- 

      The [VACUUM INTO *filename*](lang_vacuum.html#vacuuminto) command copies out
      the current state of an SQLite database into a separate file.

- 

      The [backup API](backup.html) is a C-language interface that can make a
...
is in [WAL mode](wal.html) and is being accessed from multiple processes.  But
...
alternative [sqlite3_vfs](c3ref/vfs.html) using the [sqlite3_open_v2()](c3ref/open.html) interface, an
...
send SQLITE_WARNING messages to the [error log](errlog.html) if a database file is unlinked
...
Beginning with SQLite [version 3.7.17](releaselog/3_7_17.html) (2013-05-20),
the unix OS interface will
send SQLITE_WARNING messages to the [error log](errlog.html) if a database file has 
multiple hard links.
...
in the child process, not inherited from the parent.
 ...
 Do not even call [sqlite3_close()](c3ref/close.html) on a database connection from a
child process if the connection was opened in the parent.  It is safe
to close the underlying file descriptor, but the [sqlite3_close()](c3ref/close.html)
...
- 
Changing the [PRAGMA schema_version](pragma.html#pragma_schema_version) while other database
connections are open.

- 
Using [PRAGMA journal_mode=OFF](pragma.html#pragma_journal_mode) or [PRAGMA journal_mode=MEMORY](pragma.html#pragma_journal_mode)
and taking an application crash in the middle of a write transaction.

- 
Setting [PRAGMA writable_schema=ON](pragma.html#pragma_writable_schema) and then changing the
...
the free list when [incremental_vacuum](pragma.html#pragma_incremental_vacuum) was used.  This would not cause
data loss.  But it would result in the database file being larger than
necessary.  And it would cause the [integrity_check pragma](pragma.html#pragma_integrity_check) to report
...
can be changed using the [-DSQLITE_TEMP_STORE](compile.html#temp_store) compile-time option,
or at run-time using the [PRAGMA temp_store](pragma.html#pragma_temp_store) statement.  The bug
...
This problem was [fixed](https://sqlite.org/src/info/73c2b50211d3ae26)
