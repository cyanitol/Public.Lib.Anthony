...
The number of columns in a table is limited by the [SQLITE_MAX_COLUMN](limits.html#max_column)
compile-time parameter. A single row of a table cannot store more than
[SQLITE_MAX_LENGTH](limits.html#max_length) bytes of data. Both of these limits can be lowered at
runtime using the [sqlite3_limit()](c3ref/limit.html) C/C++ interface.
...
CHECK constraints are only verified when the table is written, not when
   ...
   temporarily disabled using the "[PRAGMA ignore_check_constraints=ON;](pragma.html#pragma_ignore_check_constraints)"
...
[PRAGMA integrity_check](pragma.html#pragma_integrity_check) and [PRAGMA quick_check](pragma.html#pragma_quick_check) and sometimes
...
the [PRAGMA ignore_check_constraints=ON;](pragma.html#pragma_ignore_check_constraints) statement.
...
The rowid (and "oid" and "_rowid_") is omitted in [WITHOUT ROWID](withoutrowid.html) tables.
